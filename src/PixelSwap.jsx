...
const buildGrid = ({ width, height, pixelSize, gap, pattern, randomness, maxPixels = DEFAULT_MAX_PIXELS }) => {
  let size = pixelSize;
...
function PixelSwap({
  ...
  const grid = useMemo(
    ...
    [box.width, box.height, pixelSize, gap, pattern, randomness, maxPixels]
  );
  ...
  const interactionProps = useMemo(() => {
    if (trigger === 'hover') {
      return {
        ...
        onClick: () => {
          // Allow tapping to toggle on touch/mobile devices
  ...
  const renderLayer = (content, index) => {
    ...
    return (
      <div
        ...
        className={`pixel-swap__layer ${isShown && !transition ? 'pixel-swap__layer--active' : ''}`}
  ...
  return (
    <div
      ...
      style={{
        ...
        minHeight: box.height ? `${box.height}px` : undefined,
        ...style
      }}
